...
import {
	writeToFile,
	extractAndResolveDeclarations,
} from "./lib/save-declarations";
import {
	convertConfigToSourceSpecs,
	ParserOptions,
	SourceSpec,
} from "./lib/source-spec";
...
if (options.config) {
	...
	extractAndResolveDeclarations(sourceSpecs).then((declarations) => {
		if (options.outputDir) {
			writeToFile(
				options.outputDir,
				declarations,
				options.fileName || "config-resolved",
				!options.silent,
			);
		} else {
	...
	const sourceSpec: SourceSpec = {
		...
		excludedDirs: options.excludedDir
			? resolveDirectories(options.excludedDir)
			: [],
	...
	extractAndResolveDeclarations([sourceSpec]).then((declarations) => {
		if (options.outputDir) {
			writeToFile(
				options.outputDir,
				declarations,
				options.fileName || "declarations-resolved",
				!options.silent,
			);
		} else {
			console.log(JSON.stringify(declarations));
