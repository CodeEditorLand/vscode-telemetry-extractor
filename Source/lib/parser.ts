...
import {
	findOrCreate,
	makeExclusionsRelativeToSource,
	merge,
	populateProperties,
} from "./operations";
...
export class Parser {
	...
	private findCommonProperties(sourceDir: string) {
		const filesWithCommonProperties = this.asAbsoluteFilePaths(
			this.findFilesWithCommonProperties(sourceDir),
		);
	...
	private findFragments(sourceDir: string) {
		const filesWithFragments = this.asAbsoluteFilePaths(
			this.findFilesWithFragments(sourceDir),
		);
	...
	private findEvents(sourceDir: string) {
		const filesWithEvents = this.asAbsoluteFilePaths(
			this.findFilesWithEvents(sourceDir),
		);
	...
	private findFiles(ripgrepPattern: string, sourceDir: string) {
		const relativeExclusions = makeExclusionsRelativeToSource(
			sourceDir,
			this.excludedDirs,
		);
		const exclusions =
			...
			Promise.all(promises).then((parseResult) => {
				...
				for (const currentResult of parseResult) {
					...
					declarations.commonProperties.properties =
						declarations.commonProperties.properties.concat(
