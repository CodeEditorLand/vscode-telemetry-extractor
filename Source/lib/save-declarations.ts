...
import { OutputtedDeclarations, resolveDeclarations } from "./declarations";
...
export function writeToFile(
	...
	contents: object,
	fileName: string,
	emitProgressMessage: boolean,
) {
	if (Object.keys(contents).length === 0) {
		logMessage(
			`...no events found, skipping file emmision!`,
			!emitProgressMessage,
		);
...
export async function getResolvedDeclaration(
	...
	const parser = new Parser(
		sourceDirs,
		excludedDirs,
		options.applyEndpoints,
		options.lowerCaseEvents,
	);
...
export async function extractAndResolveDeclarations(
	sourceSpecs: Array<SourceSpec>,
): Promise<{ events: any; commonProperties: any }> {
	try {
		const allDeclarations: OutputtedDeclarations = {
			events: new Events(),
		...
		for (const spec of sourceSpecs) {
			const declarations = await getResolvedDeclaration(
				spec.sourceDirs,
				spec.excludedDirs,
				spec.parserOptions,
			);
			...
			spec.sourceDirs.forEach((dir) => {
				Object.assign(
					...
					new TsParser(
						dir,
						spec.excludedDirs,
						spec.parserOptions.applyEndpoints,
						spec.parserOptions.lowerCaseEvents,
					).parseFiles(),
				);
			});
			...
			if (spec.parserOptions.eventPrefix !== "") {
				declarations.events.dataPoints =
					declarations.events.dataPoints.map((event) => {
						event.name =
							spec.parserOptions.eventPrefix + event.name;
			...
			if (spec.parserOptions.patchDebugEvents) {
				patchDebugEvents(
					declarations.events,
					spec.parserOptions.eventPrefix,
				);
			}
			...
			allDeclarations.commonProperties.properties =
