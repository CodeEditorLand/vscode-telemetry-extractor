...
import { CallExpression, Node, Project, Symbol, SyntaxKind } from "ts-morph";
	...
	private visitNode(currentNode: Symbol, previousNode?: Symbol) {
		...
		if (type.isStringLiteral() || type.isBooleanLiteral()) {
			if (previousNode) {
				...
				if (
					this.prop_name !==
						previousNode.getEscapedName().toLowerCase() &&
					!this.prop_name.includes(
						`.${previousNode.getEscapedName().toLowerCase()}`,
					)
				) {
			...
			if (
				currentNode.getEscapedName().toLowerCase() === "endpoint" &&
				!this.applyEndpoints
			)
				return;
		...
		if (type.isStringLiteral()) {
			...
			if (
				...
				this.properties.push(
					new Metadata(
						nodeName,
						this.resolved_property[nodeName],
					).simpleObject(),
				);
			}
		}
		...
		publicLogUse.forEach((pl) => {
			try {
				...
				let event_name = pl
					...
					.getType()
					.isStringLiteral()
					? pl.getArguments()[0].getType().getText()
					: "";
