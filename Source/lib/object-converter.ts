...
export async function transformOutput(
	output: OutputtedDeclarations,
): Promise<OutputtedDeclarations> {
	// If there's no events or common properties, we emit a null object
	if (
		output.events.dataPoints.length === 0 &&
		output.commonProperties.properties.length === 0
	) {
	...
	for (const event of oldEvents) {
		...
		for (const property of event.properties as Array<
			Property | Wildcard | Metadata
		>) {
			if (property instanceof Wildcard) {
				newEvents[event.name][keywords.wildcard] = newEvents[
					event.name
				][keywords.wildcard]
				...
				for (const entry of property.entries) {
					...
					if (entry.endpoint) {
						newEntry[keywords.classification] = {
							...
							purpose: entry.classification.purpose,
							endPoint: "none",
						};
					} else {
						newEntry[keywords.classification] =
				...
				if (propetyNameChanger(property.name) === "value") {
					throw new Error(
						`Property name 'value' is reserved and cannot be used due to conflicts with the SDK. Please rename the property.`,
					);
				}
